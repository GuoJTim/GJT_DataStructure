...
void QuickSort<T,U>::quicksort(int p,int r,int(*partfunc)(std::vector<T> &,T,T)){
	if(p < r){
		int q;
...
template <class T,class U>
void QuickSort<T,U>::Sort(int(*partition)(std::vector<T> &,T,T)){
...
int QuickSort<T,U>::MedianOfThree(int p,int r){
	...
	// a <= b <= c
	int pivot_index;
	if (arr[p] <= arr[middle] && arr[middle] <= arr[r]) pivot_index = middle;
	if (arr[r] <= arr[middle] && arr[middle] <= arr[p]) pivot_index = middle;
	if (arr[middle] <= arr[p] && arr[p] <= arr[r]) pivot_index = p;
	if (arr[r] <= arr[p] && arr[p] <= arr[middle]) pivot_index = p;
	if (arr[middle] <= arr[r] && arr[r] <= arr[p]) pivot_index = r;
	if (arr[p] <= arr[r] && arr[r] <= arr[middle]) pivot_index = r;
	return pivot_index;
}
