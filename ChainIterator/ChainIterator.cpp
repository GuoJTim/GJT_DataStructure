...
template <class T>
T& CircularList<T>::ChainIterator::operator*() const{
	return current->data;
} 
...
bool CircularList<T>::ChainIterator::operator!=(const CircularList<T>::ChainIterator right){
	if (current == right.current && circularPoint){
		circularPoint = false;
		return true;
	}
	return current != right.current;
...
template <class T>
bool CircularList<T>::ChainIterator::operator!=(const CircularList<T>::ChainIterator right) const{
	return current != right.current;
}
